export module WDC {
    export class Story {
        ...
        description: string;
        url: string;
        private _rootPost: Post;

        get rootPost(): Post {
            return this.rootPost;
        }

        set rootPost(post: Post) {
            this.rootPost = post;
        }
    ...
    export class Post {
        ...
        addChoice(post: Post): void {
            post.parent = this;
            this.choices.push(post);
        }

        addChoiceAtIndex(post: Post, index: number): void {
            post.parent = this;
            this.choices.splice(index, 0, post);
        }

        // Recursive function, following the path format: '421331122121'
        traverse(path: string): Post {
            let index: number = parseInt(path[0]);
            let remainingPath = path.substring(1);
            if(remainingPath.length > 0) {
                return this.choices[index].traverse(remainingPath);
            }
            else {
        ...
        getChoice(index: number): Post {
            return this.choices[index];
        }

        removeChoice(index: number): Post {
            return this.choices.splice(index, 1)[0];
        }
    }
